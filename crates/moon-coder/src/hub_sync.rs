...
async fn create_bucket_inner(
	...
	Err(CoderError::http(endpoint, status.as_u16(), body))
}
...
async fn post_add_file(
	...
	if !status.is_success() {
		return Err(CoderError::http(endpoint, status.as_u16(), response_body));
	}
	...
	if !parsed.success {
		...
		return Err(CoderError::http(endpoint, status.as_u16(), reason));
	}
	Ok(())
}
...
fn bucket_path_for_session(_folder_path: &Utf8Path, session_id: &str) -> String {
	// One bucket per workspace, one folder root per workspace —
	// flattening the per-folder slug out of the bucket path keeps
	// the on-Hub `sessions/` listing predictable for the pi
	// viewer. Sub-agent ids carry their `sub-` prefix verbatim;
	// the parent's id isn't encoded in the bucket path either —
	// the JSONL itself carries the parent linkage.
...
fn compose_readme(workspace_basename: &str) -> String {
	...
	format!(
		...
		\n\
		Sessions land under `sessions/` as one JSONL per session, in [pi-mono](https://github.com/badlogic/pi-mono) trace shape. Hugging Face renders each file inline through the Hub's pi trace viewer.\n\
		\n\
