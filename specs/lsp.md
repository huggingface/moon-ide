...
LSP lives in `moon-core`. Nothing in the UI speaks LSP JSON-RPC directly. The frontend sees **moon-shaped** types (`LspDiagnostic`, `LspHover`, `LspCompletionList`, `LspLocation`, `LspPrepareRename`, `LspWorkspaceEdit`, `LspCodeAction`, `LspStatusEvent` — see `crates/moon-protocol/src/lsp.rs`) and Tauri commands (`lsp_open` / `lsp_update` / `lsp_close` / `lsp_hover` / `lsp_completion` / `lsp_completion_resolve` / `lsp_definition` / `lsp_prepare_rename` / `lsp_rename` / `lsp_code_action`) that forward to the broker.
...
- **`DiscoveryStrategy::NodeModules`** (TS / JS): two-tier search before falling back to `$PATH`. Tier 1 walks up from the broker's root looking for `<ancestor>/node_modules/.bin/<bin_name>` at every level — Node's own resolution algorithm, covering single-package projects and pnpm-hoisted monorepos with a single root `node_modules`. Tier 2 (only if tier 1 missed) does a bounded depth-first scan **down** from the broker's root for `<dir>/node_modules/.bin/<bin_name>`, capped at `NODE_MODULES_SCAN_DEPTH = 4` levels. This catches per-workspace install layouts (yarn / pnpm `nodeLinker: node-modules`, `apps/<pkg>/node_modules/`, `packages/<pkg>/node_modules/`) where the IDE's workspace root sits **above** the package that owns the binary. The scan skips `node_modules/` itself (no recursing into vendored deps), `.git/`, and dot-prefixed directories (`.next/`, `.cache/`, …) so the cost stays bounded on real repos. The first match wins everywhere — a project-pinned copy always beats a global install, letting a monorepo freeze a specific LSP version without affecting other projects on the same machine. Final fallback is `which::which(bin_name)`.
...
In-container binary-path resolution lives in `moon_core::lsp::server::container_binary_path`. For `NodeModules`-strategy specs it mirrors the host two-tier search: ancestor walk first, then a bounded downward scan, both rooted at the bind-mount's host path. Matches translate through the `HostMount` translator. The downward scan only descends inside the mount, so a hoisted root `node_modules` above the bound folder still falls back to host the same way it always has. `PythonVenv` walks ancestors only; for `CargoHome`- and `GoBin`-strategy specs it hands back the basename because `moon-base` installs the corresponding server (`rust-analyzer`, `gopls`) on the container's `$PATH`.
...
- **`src/lib/editor/lsp.ts`** is the CodeMirror adapter surface:
  ...
  - `lspDiagnosticsExtension()` — just `lintGutter()`; the actual diagnostics come in via `applyDiagnostics(view, perProducer)` which `Editor.svelte` calls from a reactive `$effect`. `perProducer` is the `(producer → diagnostic[])` slice for the active path — taken straight from `workspace.diagnosticsByProducer` so each `Diagnostic` carries its origin server's slot key for the [Diagnostic quick-fixes](#diagnostic-quick-fixes-lint-tooltip) prefetch.
...
- **`src/lib/editor/lspWorkspaceEdit.ts`** — shared applier for any `LspWorkspaceEdit` the broker hands back (rename today, code-action quick-fixes, future `source.fixAll` flows). Dispatches each `documentEdits` entry to `workspace.updateText` for open buffers (marking dirty + scheduling `didChange`) or `ipc.fs.readFile` + `writeFile` + `lsp.notifyFilesChanged` for closed files; returns a `{ openCount, closedCount, failures }` tally so callers can compose their own status flash. Lives in its own file so the lint tooltip's quick-fix entries don't have to import the rename UI just to apply an edit.
...
When the user parks the cursor on a squiggle, CodeMirror's lint tooltip surfaces the diagnostic message plus a short list of action buttons. Two sources feed that list:

1. **LSP-provided quickfixes** — for each producer's diagnostics on the active file, the frontend fires one `ipc.lsp.codeAction(path, producer, range, diagnostic)` per diagnostic in parallel from `applyDiagnostics()`. The broker routes the request to **the producer that emitted the diagnostic** (`spec_for_producer`); fanning out to every co-tenant would yield unrelated source-action noise — oxlint and tsgo each only know about their own diagnostics. The server's `textDocument/codeAction` reply is filtered to entries with non-empty `edit` (pure `Command` entries are dropped — we don't run `workspace/executeCommand`), then projected into `LspCodeAction[]` and cached per `(path, producer|line:char|code|message-prefix)`. A second `setDiagnostics` dispatch lands once the prefetch resolves so the tooltip rerenders with the action list. A generation counter discards stale prefetches when the user swaps files mid-fetch.

   For oxlint, this surfaces three quickfixes per warning: an autofix when the rule is fixable, plus "Disable `<rule>` for this line" (inserts `// oxlint-disable-next-line <rule>`) and "Disable `<rule>` for this whole file" (inserts `// oxlint-disable <rule>` at the top). `tsgo` advertises a smaller set; rust-analyzer's quickfixes (`Fill match arms`, `Add type annotation`) come through the same path.

2. **"Fix in coder"** — always present, no IPC. `coder.fixDiagnosticInCoder(...)` opens the coder panel, attaches the file/range as a selection chip (so the model sees the squiggled span), and seeds the composer draft with `Fix [source code]: <first line of message> @path:line` — short on purpose, the user edits before sending. The escape hatch when the linter has no programmatic fix to offer (or when the user wants a wider rewrite than the rule's fix gives them).

Both kinds apply through `applyWorkspaceEdit` — the same surface the rename flow uses. Open buffers get rewritten in memory (auto-saves via the normal dirty-buffer path), closed files go through the workspace host with a follow-up `notifyFilesChanged`.
...
- **Restart / crash-recovery reopens through the slot's covered set.** The slot key (`"oxlint"`) doesn't appear as anyone's file-language id, so a `restartLsp("oxlint")` that walks `openFiles` filtering by `lspLanguageFor(path) === "oxlint"` would silently match zero files and the linter would come back up empty. The frontend's [`lspSlotCoversFile`](../src/lib/editor/lspLanguage.ts) maps a slot key to the set of file-language ids the slot governs (oxlint → the four JS/TS ids; tsgo → the same set; rust-analyzer → just `"rust"`); both `restartLsp` and `#reopenActiveForLanguage` (the `lsp:status crashed` listener) use it to pick the buffers to re-`lspOpen`.
- **One workspace folder per nested `.oxlintrc.json`.** oxlint anchors its config discovery on `workspaceFolders`, **not** on a walk-up from each file. A monorepo like `boardgamers-mono` with `.oxlintrc.json` at `apps/api/`, `apps/game-server/`, `packages/models/`, and `packages/utils/` needs each containing directory advertised as its own workspace folder, otherwise the LSP falls back to oxlint's built-in defaults — which means `options.typeAware: true` doesn't take effect (no `typescript-eslint(no-base-to-string)` squiggles), per-rule overrides like `eslint/no-constant-condition: ["error", { "checkLoops": false }]` are ignored (`while(1)` flares for nothing), and the editor disagrees with the project's own `oxlint --fix` script. [`discover_oxlint_workspace_folders`](../crates/moon-core/src/lsp/server.rs) does a `.gitignore`-aware bounded scan from the workspace root (depth 5, enough for `apps/<pkg>/<sub>/`), the broker passes the result to `LspServer::spawn` as a host-relative `Vec<String>`, and `initialize` translates each entry through `PathTranslator::absolutise` so container-routed servers see `/workspace/<basename>/<rel>` URIs. Language-server slots (tsgo, rust-analyzer, ty, gopls) get the single-entry `[""]` (the root) — they index from `rootUri` regardless and a wider list buys nothing.
- **`workspace/configuration` returns `[{}, {}, …]`, not `null`.** Once oxlint has the workspace folders, it requests its per-folder LSP settings via `workspace/configuration`. The default auto-reply in [`client.rs`](../crates/moon-core/src/lsp/client.rs) was `null` for every server→client request — fine for `client/registerCapability` and `workspace/diagnostic/refresh`, but oxlint reads `result[i]` directly and a `null` makes it silently fall back to defaults that never honour the on-disk `.oxlintrc.json`'s `options.*` keys. Replying with an array of empty objects (one per requested item) is the spec-correct "no per-folder LSP overrides — server, use the disk config you already loaded" answer, and it's what every other server we run accepts without complaint. Combined with the workspace-folder fan-out above, this is what makes the editor's diagnostics line up with the project's own `oxlint --fix` script in monorepos.
- **Backend safety net via `update`-as-`open`.** [`LspBroker::update`](../crates/moon-core/src/lsp/broker.rs) routes through [`LspServer::open`](../crates/moon-core/src/lsp/server.rs) (which already merges first-time-open and subsequent-change), not a separate `update` that silently drops unknown rel-paths. A respawned server (whether evicted by the broker's `is_alive()` check or by a manual `shutdown_language`) starts with an empty `docs` map; the very next debounced `lsp_update` from in-flight typing then becomes its `didOpen` instead of disappearing into a void. Without this, the linter's diagnostics would freeze on the pre-crash snapshot until the user switched tabs and back — the original [test plan 0078 step 6](test-plans/0078-oxlint-lsp.md) only checks the crash → restart → fresh-publish path on the active file, and a passive (in-place editing only) recovery would have escaped that test.

`OXLINT_LANGUAGES` (in `crates/moon-core/src/lsp/server.rs`) is the wire-up: the four JS/TS file language ids on which the broker spawns oxlint as a co-tenant. Adding more linters in the future is the same template — a new `LspBinarySpec`, a new `<linter>_LANGUAGES` set, a new branch in `Broker::lint_spec_for`, and a new branch in `lspSlotCoversFile` on the frontend (the only place where the slot↔file mapping isn't generated from the backend). We don't need a generic "registry" abstraction yet; one linter ships, more get added when the team asks.
