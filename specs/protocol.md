...
- `next_edit_complete({ params }) -> NextEditCompleteResult` — builds a Sweep-style prompt (original / current / updated 21-line windows; see [Sweep next-edit post](https://blog.sweep.dev/posts/oss-next-edit)) and `POST {baseUrl}/completion`. The result reports `from_line` / `to_line` covering the full window, **except** when llama-server didn't stop on the trained `<|file_sep|>` word and the reply is shorter than the window — that's truncation (`n_predict` cap, EOS, etc.), so `to_line` is narrowed to `from_line + reply_lines - 1`. Trusting the full window in that case made the frontend overwrite un-replaced lines below the edit; narrowing flips the failure mode from "destroy unrelated lines" to "miss an end-of-window deletion the model intended".
