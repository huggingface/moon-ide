...
- **Periodic auto-fetch** (see `specs/test-plans/0064-*.md`). `WorkspaceHost::git_fetch` shells out to `git fetch --quiet --no-tags` with prompts disabled (`GIT_TERMINAL_PROMPT=0`, blanked `GIT_ASKPASS` / `SSH_ASKPASS`) and a 30s deadline. The frontend wires a 3-minute periodic loop (matches VSCode / Cursor's `git.autofetchPeriod` default), an initial fetch ~5s after startup, plus focus / folder-switch nudges throttled to a 30s minimum. Fetch only moves remote-tracking refs, so the followup is just `refreshGitBranch` (cheap) — the SCM panel's "Sync Changes" button surfaces from the refreshed `gitBranch.behind` count without any other refresh fanout. Failures (offline, auth, no upstream) downgrade to `console.debug`; the loop pauses when the document is hidden.
