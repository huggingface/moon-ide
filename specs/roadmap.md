...
- **Diff view** via `@pierre/diffs` (see `specs/test-plans/0032-*.md`). `HEAD` content is pulled via a new `fs_git_head_content` command (`git show HEAD:<path>`), rendered in split layout by `DiffView.svelte`. A modified file's **View diff** entry (tree context menu + palette command) opens a dedicated **diff tab** at synthetic path `moon-diff:<realPath>`, which coexists with any regular editor tab for the same file — `Alt+Left` walks back to the editor view naturally. Deleted rows continue to open as a single tab in diff mode (there's no editable counterpart). The diff tab's "after" side prefers the live editor buffer when one is open, otherwise falls back to a one-shot `readFile`. Scope is deliberately minimal — read-only, `HEAD` vs working tree, no staging / no branch compare — matching what the team actively needs right now.
