...
- **No abort affordance.** A merge that ends in conflicts leaves the user in a `MERGING` state with no in-app "abort merge" button. They handle it from a terminal (`git merge --abort`). Phase 5's full conflict UI will close this.
