...
- **`refreshGitStatus` / `refreshActiveFolder` / fs-watch** — auto-fetch calls `refreshGitStatus` only when `refreshGitBranch` reports a HEAD SHA different from the pre-fetch snapshot. That's the safety net for external ref moves the fs watcher can't see (`git reset --soft`, external commit, `git switch` to identical content — all rewrite `.git/refs/heads/<branch>`, nested under the non-recursive `.git/` watch). When HEAD didn't move (the common case — a fetch only moves remote-tracking refs), `refreshGitStatus` is skipped as before.
