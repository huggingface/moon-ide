...
1. **Apply `visibleOnce` to `ToolBody*` components**. Same pattern as ship 7 for `CoderMarkdown` — gate the per-row decorated content on first-intersection. Likeliest follow-up if `coderPanel.update → firstFrame` ever creeps back above ~500 ms.
...
- Applying the same `visibleOnce` gate to `ToolBody*` (read-file, edit-file, grep, write-file, …). Each of those components also runs a per-row decorated render (syntax highlighting on read/write, diff hunks on edit, …), and we'd expect a further cold-cache win from gating them on visibility too. Holding off until we see if the residual ~310 ms style recalc after ship 6 was dominated by markdown (ship 7 removes that) or by tool bodies (a fresh recording will tell). Worth a follow-up if `coderPanel.update → firstFrame` still exceeds ~300 ms in the next trace.
