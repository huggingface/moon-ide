...
4. Keep going until the ring crosses ~80 %. Expected: at the **next** iteration's start, a "compacting…" pulse appears as a transcript row **inline at the current bottom of the conversation** (italic; ring also pulses). Within a few seconds it flips in place to a `<details>` titled `Compacted N earlier messages into a summary`. Click it: the synthetic summary is visible, covering user intent, decisions, and current state. Send another prompt: the compaction row stays where it was and the new turn renders **below** it (it does not jump to the bottom).
...
- **Reopen after compaction keeps the recent turns and the disclosure.** Drive a session into compaction (step 4), let it run a couple more turns, then close and reopen the session from the list. Expected: (a) the compaction `<details>` reappears inline at the spot it folded (collapsed), and (b) the agent still has the last ~6 user turns in context — ask `what was I just doing?` and verify it references the most recent work, not only the summary. On disk, the `Compaction` record carries `details.messages_kept` > 0.
