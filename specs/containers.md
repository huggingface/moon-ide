...
## Project services live alongside, not inside

The workspace container is the user's editing/build sandbox.
Anything else the project needs to come up — postgres, redis,
mongo, an internal HTTP service, a queue — runs as a separate
container on the **host's** Docker daemon, declared in the
project's own `docker-compose.yml` and surfaced to moon-ide
via compose's top-level [`include:`](https://docs.docker.com/compose/multiple-compose-files/include/)
directive.

The generated `<workspace>/.moon/compose.yaml` looks like:

```yaml
# .moon/compose.yaml — generated by moon-ide
name: moon-ws-${WORKSPACE_HASH}

include:
  # Pulled in automatically when moon-ide spotted a sibling
  # docker-compose.yml at workspace open. Edit, reorder, or
  # remove these as you like — moon-ide doesn't rewrite this
...
`docker compose -f .moon/compose.yaml up -d` brings up
**both** the included services and the `dev` workspace
container, all as siblings on the user's host daemon, all on
one shared compose network. From inside `dev`, the user's
`bun run dev` reaches the project's services by service name
(`mongo:27017`, `redis:6379`); from the host, anything the
project's compose declared `ports:` for is reachable as
before.
...
That shape — services in compose, app on the host — maps
directly:

- moon-ide generates `<moon-landing>/.moon/compose.yaml` with
  `include: [../docker-compose.yml]` and a `dev` service.
- `docker compose up -d` brings up all eleven containers (the
  project's ten plus `dev`), the user logged into
  `registry.internal.huggingface.tech` on their host's
  daemon picks up the private images transparently.
- The user's app continues to run wherever it ran before —
  on the host (reaching services through published host
  ports, exactly as today) or in `dev` (reaching them by
  service name, no `host.docker.internal` indirection).

Crucially, **nothing in moon-landing's compose has to change**
for this to work. `include:` composes the two files into one
project; the project's services and the workspace's `dev`
share the same default network without further coordination.
...
Compose `include:` gives us the orchestration value without
the kernel surface. The workspace runs unprivileged with
Docker's default capability set; the host's daemon — which
the user already trusts to manage all their other containers
— runs the services. ADR 0008 records the full reasoning.

### Multi-repo workspaces

When a workspace holds multiple repos that each declare a
compose file, the generated `compose.yaml` includes them in
turn:

```yaml
include:
  - ../moon-landing/docker-compose.yml
  - ../moon-bot/docker-compose.yml
```

Compose merges them into one project. Service-name collisions
between two repos surface as a startup error — the user
either renames a service in one repo or scopes them with
profiles. We don't try to silently namespace. (When this
turns out to bite, the planned escape hatch is per-repo
profile prefixes, but that's a problem to solve when it
shows up, not a problem to engineer for now.)
...
and emits, for each entry:

- A bind mount on the `dev` service:
  `<absolute>:/workspace/<basename>` — no `../` games, the
  file no longer lives near any repo so absolute paths are
  the only sensible form.
- An `include:` of any `compose.yaml` / `docker-compose.yml`
  discovered at the folder root or one level deep, again
  absolute.
...
- **Add a folder** (sidebar `+`, welcome screen, command
  palette): the IDE adds it to `bound-folders.json`, regenerates
  `compose.yaml`, and — if the project is currently `running` —
  runs `docker compose up -d --wait` to apply the diff.
  Unchanged services stay running, the `dev` container is
  recreated because its mount list changed, new project
  services start. Pre-opt-in (no `compose.yaml` yet) is a
  no-op: the file is materialised on first "Set up".
- **Remove a folder** (per-bar `×`, with confirm): the inverse.
  `compose.yaml` is regenerated; if the project is running, the
  next `up -d --wait` recreates `dev` without the dropped mount
  and prunes orphaned project services on a subsequent
  `--remove-orphans` (deferred until somebody asks — leftover
  services from a removed folder are visible enough in
  `docker compose ps` that auto-pruning isn't worth the
  surprise).
- **Switch active folder**: zero-cost on the container side.
  The compose project doesn't move; only the IDE's
  per-folder UI state (tab strip, file tree, scroll position)
  swaps.
