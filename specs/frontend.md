...
- **Lazy descendant fetch.** When the user expands a collapsed-ignored directory in the file tree, `FileTree.svelte` issues `fs_collect_paths_under(rel, max_depth=1)` to fetch its direct children and batch-adds them to Pierre. Each loaded sub-directory is itself recorded as lazy, so drilling deeper into `node_modules/foo/bar/` re-issues the command at the next level. Loaded paths are tracked separately from `workspace.paths` and re-unioned into the merged path set on every refresh so the path-set effect's `applyPathsDiff` doesn't churn them. Folder switches reset the lazy bucket; same-folder watcher refreshes preserve it.
