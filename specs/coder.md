...
| `bash`       | `(cmd, timeout_ms?) -> { cmd, target, stdout, stderr, exit_code }`                                   | Local folders: `sh -lc <cmd>` rooted at the folder. Devcontainer folders: `docker exec -w <container_cwd> <name> sh -lc <cmd>` (no `-it` — captured I/O, not a TTY). `target` echoes `"host"` / `"container"` so the panel and tool result agree. Same convention as `moon-terminal`'s `TerminalTarget::Container` |
