...
| `edit_file`      | `(path, find, replace, occurrence?) -> { path, bytes_written, mtime_ms, occurrence, total_matches }`                | `find` is an exact substring (whitespace significant); empty `find` rejected; non-unique match without `occurrence` throws so the LLM retries with more context. Same path routing as `read_file`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
