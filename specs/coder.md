...
A wire-shape caveat for Anthropic's native API: its `usage.input_tokens` is _only the non-cached portion_ of the prompt — `cache_read_input_tokens` and `cache_creation_input_tokens` are reported alongside it, not as a sub-breakdown. On a long, heavily-cached session that collapses `input_tokens` to a handful of tokens while the real prompt sits in `cache_read_input_tokens`. Our Anthropic adapter (`merge_usage` / `into_assistant_response` in `crates/moon-coder/src/anthropic.rs`) rolls the cache portions into `prompt_tokens` before handing the usage to the runner, so the ring denominator and the compaction trigger both see the full input. The OpenAI-compatible path needs no such fixup — its `prompt_tokens` already covers cached tokens.
...
| `edit_file`  | `(path, find, replace, occurrence?) -> { path, bytes_written, mtime_ms, occurrence, total_matches, match_mode }`    | `find` is matched against the file in five stages: (1) exact byte substring; (2) once more after unescaping literal `\n` / `\t` in `find` (escape-leakage from the model's tool-call JSON); (3) backslash-run-collapsing — collapse every run of consecutive `\` bytes to a single `\` on both sides, find the match, splice the file's original byte range and drop `replace` in verbatim; catches the "regex / template-literal escape-count is off by one" failure mode in both directions (model under- and over-escaped); (4) line-aligned indent-tolerant — strip per-line leading whitespace from both sides, find a line-aligned window, splice the file's original byte range and re-indent `replace` to match the file's indent at that point; (5) whitespace-collapsing — normalise every run of ASCII whitespace context-sensitively (kept as one space between word chars, dropped adjacent to punctuation / EOL) on both sides, find the match, splice the file's original byte range and drop `replace` in verbatim. Stage 5 catches the "formatter rewrote a multi-line call into one line" failure mode that stage 4 misses. `match_mode` in the result is `"exact"` / `"fuzzy_unescape"` / `"fuzzy_backslash"` / `"fuzzy_indent"` / `"fuzzy_whitespace"` so the model can tell which path took. Empty `find` rejected. Non-unique match without `occurrence` throws (the error lists matching line numbers so the model can disambiguate). Same path routing as `read_file`. Fuzzy paths trust format-on-save (editorconfig pre-save + lint-staged) to normalise the spliced bytes — the model still aims for byte-exact, fuzzy is the safety net.                        |
...
Every bound workspace folder can host **multiple concurrently-running** sessions (see [ADR 0016](decisions/0016-coder-concurrent-sessions.md)). Each session has its own in-memory chat history, its own running-turn cancel handle, its own composer draft / attachments, and its own context-usage ring. Switching the active workspace folder doesn't touch other folders' sessions; clicking `+` for a new session or opening another session in the same folder doesn't touch the previously-visible session's running turn either. An agent kicked off in folder X session A keeps streaming events into A's UI bucket while the user starts session B in the same folder, browses folder Y, or both.

The backend keeps the per-folder runtime state in `CoderState.sessions_by_folder: HashMap<Utf8PathBuf, Arc<FolderSession>>`. Each `FolderSession` is a map of `Arc<SessionRuntime>` keyed by session id plus a `visible: Option<String>` pointer the panel uses to know "which one is the user currently looking at". A `SessionRuntime` is a `Mutex<Session>` (the in-memory chat history) plus a `Mutex<TurnState>` (the cancel token for its in-flight turn, if any). Lazy creation: a `FolderSession` materialises the first time a command needs one for that folder, and a `SessionRuntime` materialises either when `new_session` / `open_session` brings it forward or when an inbound event for an unknown session id arrives. We never garbage-collect entries on folder unbind — they sit in the map until process exit. Cheap; rebinding the same folder gets the same in-memory state back.

Tools captured by a running turn close over the **session's bound folder**, not the live `WorkspaceRegistry::active_folder()`. That's how "agent in folder X stays bound to folder X" actually works at the tool layer: the spawned `run_turn` task carries an `Arc<SessionRuntime>` plus the folder string, and its `ToolContext` uses that folder for every dispatch. Switching active folder mid-turn cannot redirect tool calls, and switching the visible session inside a folder cannot either.

`abort` operates on the **active folder's visible session** turn only. Stopping a background turn — either in the same folder's other sessions or in a different folder entirely — requires making it visible first (clicking its row in the sessions list, or switching to that folder). Sign-out is the one global exception: it cancels every running turn across every session in every folder, since the auth identity backing them just went away.

Per-folder hydration on launch: `AppState.coder.last_session_by_folder: Map<folder, sessionId>` records the last-visible session per project. The panel restores the active folder's entry on first mount and again whenever the user switches back to a folder it hasn't visited yet in this session — the pointer becomes the folder's `visibleSessionId` after restore. The pointer is updated by both `coder_open_session` (explicit click in the sessions list) **and** `coder_send` (so a fresh `new_session` + first message refreshes the pointer — without that nudge the entry would still point at whichever transcript the user had open before clicking "new"). If the pointer goes stale (session JSONL deleted out-of-band, data dir wiped), `#hydrateSession` swallows the missing-file error and falls through to the sessions list rather than mounting an inline error row; the first subsequent `open_session` or `send` overwrites the stale entry, so the failure mode self-heals. Background turns don't survive a process restart — only the _visible_ session per folder hydrates on launch, since there's nothing in-flight to preserve anyway.
...
- **`User`**, **`Assistant`**, **`Tool`**, **`Compaction`** map natively to pi's `user` / `assistant` / `toolResult` messages and `compaction` row. An assistant turn whose response carries no text, no thinking, and no tool calls — typically a provider that bailed mid-stream or streamed only a usage chunk — is **not persisted**: re-inflating one into a `ChatMessage::Assistant { content: None, tool_calls: [] }` on the next prompt trips Anthropic's `text content blocks must contain non-whitespace text` 400. Historical sessions with that shape on disk get the same treatment on load (`pi_wire_to_records` drops the row; any sibling `usage` block still lands as a stand-alone `Usage` record). The wire-side `anthropic::translate` carries a belt-and-braces version of the same check so a stray empty assistant message slipping through never reaches the provider.
- The pi `provider` + `model` fields on an `assistant` row reflect the **actual route that served the round-trip** (`anthropic` / `openrouter` / `huggingface` / a user-provider id), not the session header's seed `model`. The seed was set once at session creation and stays frozen — the per-message stamp is what `provider/model` was active when the assistant replied, so flipping providers mid-session shows up correctly per turn in the pi trace viewer. Pre-existing sessions written before this stamp shipped fall back to the header's seed on reload.
...
The header's `updated_at_ms` is **frozen at first-persistence time**: the transcript is append-only and the header is written verbatim only on file creation, so it never advances on disk. Recency for the sessions list comes from the file's **mtime** instead — `load_summary` (and `load`) read it via `file_mtime_ms` and use it as `updated_at_ms`, falling back to the frozen header value only if the mtime can't be represented. mtime is "last write to the file", which is exactly the activity signal we want, and it's free: every `append_record` touches it, so there's no extra write and the append-only crash-safety stays intact (a header rewrite-on-append would have made every append an O(filesize) read-modify-write and lost atomicity — rejected for that reason). The in-process path is independent: `Coder::send` / `drain_pending_steers` / auto-rename bump the in-memory header and the frontend re-sorts the list live, so the row floats to the top the moment a message is sent without waiting for a disk round-trip; the mtime read only governs cold reads after a relaunch. None of this touches the inference request path — wire messages are built from `messages`, never the header.
...
If the user stops the coder mid-tool (Esc, panel close, IDE quit) the assistant's `tool_calls` already hit disk but the matching `tool` record never did — the dispatcher was cancelled before it could emit one. Without recovery, the next turn would crash inside the provider (every chat-completions API rejects an `assistant.tool_calls` without its matching `tool` results — Anthropic returns HTTP 400 "`tool_use` ids were found without `tool_result` blocks", OpenAI does the same in a slightly different shape) and reopening the session would render a forever-"running" tool row. Two complementary recovery points handle the two ways the orphan can surface:

- **At abort time (same-process, in-memory session).** When `run_turn` returns `Err(Aborted)`, the spawn closure in [`Coder::send`](../crates/moon-coder/src/runner.rs) calls `recover_in_memory_orphans` **before** emitting `CoderEvent::Aborted`. It walks the live `messages` slice, finds every assistant `tool_call` id without a matching `Tool` message later in the slice, and for each: appends a synthetic `ChatMessage::Tool` carrying `INTERRUPTED_TOOL_RESULT_JSON`, persists a matching `SessionRecord::Tool` to the JSONL (so the on-disk shape stays consistent with memory), and emits `CoderEvent::ToolResult { is_error: true, ... }` so the panel flips the row to errored. Without this, sending a follow-up prompt in the same in-memory session immediately after Esc would ship an unpaired `tool_use` to the provider and 400.
- **At reload time (process restart, or opening another session and clicking back).** [`open_session`](../crates/moon-coder/src/runner.rs) walks the records once, scopes any `tool_call` id with no later `tool` record as orphan, synthesises a `ChatMessage::Tool` for each (appended to the rebuilt `messages` slice so the next turn has a valid history), and emits a synthetic `CoderEvent::ToolResult { is_error: true, ... }` per orphan after the replay loop. This path covers the IDE-quit / crash window where the abort-time recovery never got to run; once the at-abort-time path has done its job, reload-time recovery finds zero orphans and is a no-op (idempotent).
...
- **Revert to here** (undo-arrow icon) — drops that user message and everything after it from both the on-disk JSONL and the in-memory chat history. Confirmed by a modal because it permanently rewrites the transcript.
- **Edit & resend** (pencil icon) — same truncation, but the dropped prompt's text is loaded back into the composer (focus lands there) so the user can tweak it and re-send. No confirm dialog: the text isn't lost, it lands in the composer, and a modal would make an "edit" gesture feel heavier than it is.

Both route through `coder_revert_to_message(user_ordinal)`. The anchor is the **0-based ordinal of the user message among the transcript's user records**, in order — _not_ a row id. User-message ids are minted fresh on every replay (`new_message_id()` in the runner), so they aren't reload-stable; the ordinal is. Steers count as user records (they render as `user` rows and persist as `SessionRecord::User`), so the frontend's ordinal counter walks `kind: 'user'` rows in order, **skipping queued (not-yet-drained) steers** — those aren't on disk yet and so aren't in the backend's `User`-record count. The affordances are hidden on queued rows and while a turn is running.

Backend mechanics ([`sessions::truncate_before_user_record`](../crates/moon-coder/src/sessions.rs) + [`Coder::revert_to_message`](../crates/moon-coder/src/runner.rs)):

1. Refuse if the visible session's turn is in flight (checked under the turn lock) — rewriting the transcript under the running loop would corrupt its next iteration. The panel also hides the affordance while busy; the backend check is belt-and-braces.
2. Load the records, find the `user_ordinal`-th `User` record, and rewrite the JSONL from the header plus every record _before_ it. Folded `usage` blocks come back off `load` as stand-alone `Usage` records, so re-serialising them stand-alone round-trips cleanly. An out-of-range ordinal errors (`CoderError::Internal`) and leaves the file untouched. Reverting the 0th message empties the transcript but keeps the header line, so a subsequent send appends rather than re-writing a header.
3. Drop the mounted `SessionRuntime` and call `open_session(id)` — that rebuilds the in-memory `Session` from the now-shorter JSONL and replays the trimmed transcript to the panel through the existing event path, so there's no second copy of the replay logic to keep in sync.

The dropped prompt's `{ text, images }` comes back as `RevertedMessage` for the edit-and-resend prefill; the trimmed transcript itself repaints from the `open_session` replay events, not the return value.
...
context block ships on its own); empty + empty is a no-op. We
deliberately don't ship an implicit "active file" hint with
every turn — if the model needs the focused file it can ask,
and the user can always pin it explicitly with `Ctrl+L`.
...
A workspace starts unbound. The coder panel header carries its
own cloud-sync icon button (next to the model-settings cog) that
opens
[`HfBucketSettingsModal.svelte`](../src/lib/components/HfBucketSettingsModal.svelte).
When no binding exists, the settings modal exposes a single
"Connect to Hugging Face" CTA that opens
...
After create, `autosync` defaults to **false**. Reopening the
trace-sync modal now shows the connected state with a checkbox
the user can flip on — connecting the bucket alone doesn't
trigger any pushes. The same cloud-sync header icon shifts to an
accent colour so the user can see at a glance the workspace is
bound.
...
The directory key is the same `<basename>-<fnv8>` slug
[`crate::sessions::project_slug`](../crates/moon-coder/src/sessions.rs)
uses under `<XDG_DATA_HOME>/moon-ide/coder-sessions/`, so local
↔ Hub paths line up 1:1. The bucket holds nothing else: no
`sessions/` prefix, no manifests, no attachments — the bucket
itself is single-purpose (its name is literally `*-traces`), so
an extra level of nesting would just be noise.
...
Each folder bound to the workspace gets its own directory at the
bucket root (`<folder-slug>/`), holding one JSONL per coder
session in pi-mono trace shape. Hugging Face renders each file
inline through the Hub's pi trace viewer.
...
5. `POST /api/buckets/<owner>/<name>/batch` an NDJSON `addFile`
   line binding the hash at `<folder-slug>/<id>.jsonl` with the
   JSONL content type.
...
The trace-sync settings modal has a Disconnect button. It clears
...
1. **Pre-call** (Estimate). Right before sending the prompt to the provider, the runner emits a `TokenUsage` with `prompt_tokens` anchored on the previous turn's exact figures (`last_usage.prompt_tokens + last_usage.completion_tokens`) plus a bytes/4 estimate of whatever was appended after the last assistant message (new user prompt and/or tool results), and `completion_tokens = 0`. The ring jumps the moment the user hits send (or a tool result lands), instead of waiting for the SSE stream to finish — and crucially it doesn't shrink back to a cruder bytes/4 of the whole array between turns, which was visibly wrong on Anthropic where the tokenizer counts more than bytes/4 suggests. First turn (no `last_usage`) and post-compaction (which clears `last_usage`) fall back to plain bytes/4 of the message array.
...
2. Calls the **cheap model** (see [Model picker](#model-picker)) with a fixed system prompt that asks it to summarise the prefix (`messages[1..cut]`) — covering user intent, decisions, files touched, current state, and what to do next. Output is plain markdown; the call is non-streaming and has no tools. **The prefix is chunked** so no single summary call exceeds the cheap model's own context window: the rendered messages are packed into chunks each under ~55 % of that window, every chunk is summarised independently, and if there was more than one chunk the partial summaries are folded together with a final merge pass (recursing if the concatenation is itself too big). An oversized single message (a huge tool result / pasted file) gets its own chunk, truncated to the budget. This matters because on a long, heavily-cached session the prefix can be far larger than the cheap model's window — e.g. a 700k-token history summarised by a 200k-window Claude model. The earlier single-shot implementation 400'd on every such call, so compaction silently never ran and the session grew unbounded past the cap. Per-chunk failures are tolerated (a partial summary beats none); the pass only gives up if _every_ call fails.
...
The on-disk JSONL transcript is **not** rewritten — the full history stays on disk so pop-out / debug / audit see everything; only the in-memory prompt the next round-trip sends gets compacted. As a consequence the on-disk file can be longer than what the agent currently has in front of it; that's intentional. To keep replay reaching the same compacted shape, the runner also appends a [`SessionRecord::Compaction { summary, messages_compacted, messages_kept }`](../crates/moon-coder/src/sessions.rs) record at the point the live fold happened. `messages_kept` is how many trailing messages rode through unchanged (the recent user turns and their replies, `messages[cut..]`). It is **load-bearing for replay**: the kept turns are persisted to disk _before_ the `Compaction` record (they happened earlier in the run), so on reload replay rebuilds the full transcript linearly — kept turns included — and at the `Compaction` record folds everything _except_ the last `messages_kept` messages. Folding the whole prefix instead (an earlier bug) silently dropped the recent turns the live pass deliberately retained, so a reopened session lost its last ~6 turns of working context. Without the record at all, reopening would re-inflate the entire pre-compaction transcript and the next turn would instantly trip the provider's context-length cap.

After a fold, the runner re-anchors `last_usage` on a bytes/4 estimate of the _compacted_ prompt rather than clearing it to `None`. Clearing would skip the compaction check on the next iteration entirely (it early-returns on missing usage), so a single pass that didn't get under the threshold — a large summary plus heavy kept turns — would send one over-budget prompt to the provider before the next response's usage re-armed the guard. Seeding the estimate keeps the guard live and re-fires immediately if we're still over.
...
The frontend turns these into an **interleaved transcript row** ([`CoderRow`](../src/lib/coder.svelte.ts) of `kind: 'compaction'`) at the point in the transcript where the fold happened — a "compacting…" pip while the fast-model call is in flight, flipping in place to a `<details>` showing the summary once `compaction_complete` lands. Rendering it as a real row (rather than a single field pinned to the bottom of the transcript) means it scrolls away under subsequent turns, a second compaction in the same session appends a second row instead of overwriting the first, and the disclosure **survives a reopen**: replay re-emits the `started` + `complete` pair for each `Compaction` record (the `complete` carries `prompt_tokens_after: 0` as a replay sentinel the ring ignores), so the summary the agent is actually running on stays visible after reload. The ring also pulses while compaction is running.

If every chunk's summary call fails or the model returns an empty summary, compaction is skipped (the agent keeps going with the uncompacted prompt; a warn-level log gets dropped) and `compaction_complete` fires with an empty summary so the UI's "compacting…" pip clears.

One edge case the pass can't fix: if the **6 retained recent turns** alone already exceed the standard model's window (a burst of huge tool results), compaction can shrink the prefix but can't get under the limit, and the loop will keep re-compacting the same (now-minimal) prefix each turn. That's rare and self-limiting — the prefix is already a single summary message — but it means compaction is not a guarantee against an over-window prompt, only a strong mitigation. After a fold the runner re-anchors `last_usage` on an estimate of the compacted prompt (not `None`), so the guard stays armed and re-fires immediately if a single pass didn't get under threshold.
...
When a sub-agent spawns, `run_subagent` kicks off a best-effort `git fetch` against the **target folder's** [`WorkspaceHost`](architecture.md#workspacehost-phase-2) ([`subagent::kick_off_subagent_fetch`](../crates/moon-coder/src/subagent.rs)). The motivation: the human-facing active folder already has a periodic auto-fetch loop (test plan [0064](test-plans/0064-git-auto-fetch.md)), but a sibling folder a sub-agent is dispatched against may not have been fetched in a long time, so a sub-agent doing `git log origin/main` / `git diff origin/main...HEAD` (exactly the review-branch commands the system prompt recommends) could read stale remote-tracking refs. The pre-fetch closes that staleness window precisely when it would bite — demand-driven, so an idle workspace pays nothing.

Three properties keep it cheap and safe:

- **Fire-and-forget.** The fetch is `tokio::spawn`ed, never awaited. A fetch is up to a 30s network call (capped inside `git_fetch`) and a sub-agent's opening tool calls are almost never an `origin/main` read, so it races ahead of the sub-agent's reasoning and lands before it matters rather than adding latency to the dispatch.
- **Per-folder throttled.** A process-global `folder-root → Instant` map (`SUBAGENT_FETCH_THROTTLE` = 5 min) collapses bursts: a parent that fans out four `task` calls against one folder triggers one fetch, not four, and a chatty session re-delegating to the same sibling doesn't re-fetch on every spawn.
- **Fetch-only, so no conflict risk.** `git fetch` moves only remote-tracking refs (`refs/remotes/origin/*`) — never the working tree, index, or `HEAD`. There's nothing to conflict with and nothing to surprise an in-flight edit. We deliberately don't `git pull` / merge the target folder's checked-out branch: that would mutate a working tree nobody asked us to touch (the same reason test plan 0064 rejected auto-pull for the active folder). Failures (offline, no upstream, auth refused, timeout) are swallowed by `git_fetch`'s quiet path with a `tracing::debug!`; the sub-agent runs against whatever refs it has, exactly as before.
...
[`tools::ToolRegistry::resolve_target`](../crates/moon-coder/src/tools.rs) classifies each path argument and returns a `ResolvedTarget` — either `InWorkspace { folder, relative }` (dispatch through that folder's `WorkspaceHost`) or `OutOfWorkspace { abs_path }` (dispatch through the container-aware out-of-workspace primitives):

1. **Absolute path under a bound folder's root**: `InWorkspace`, routed to that folder. This is the everyday host-mode form — the system prompt's "Bound folders" section advertises each folder by its absolute host path, and the model joins file-relative paths onto it. When multiple bound folders' roots match (a strict-ancestor case the file tree allows), the longest match wins so an inner folder's relative addressing stays correct.
2. **Synthetic active or sibling** (`/workspace/<name>/...`): `InWorkspace`, looking up the basename in the workspace registry and routing to that folder's [`WorkspaceHost`](architecture.md#workspacehost-phase-2). This is the container-mode form the prompt advertises while the workspace shell container is `Running`. An unbound `<name>` errors with `CoderError::ToolFailed` carrying the list of currently-bound folders so the model can self-correct without another guess turn. Kept available in host mode too (no behaviour gate by mode at the resolver), but the prompt only advertises it when the container is up.
...
5. **Absolute path outside every bound folder root** (`/etc/hosts`, `$HOME/.config/...`, a build artifact elsewhere on disk): `OutOfWorkspace`. The four fs tools route these through container-aware primitives that mirror `bash`'s target — `docker exec` into the workspace shell container when it's `Running`, the host filesystem otherwise. There is **no** workspace-root gate here: arbitrary-path read/write/list is the point (see [ADR 0025](decisions/0025-coder-arbitrary-path-fs.md)). Format-on-save does not apply to out-of-workspace writes (no `.editorconfig` / lint-staged cascade outside a project), so they land the model's exact bytes and skip the turn-end `FormatQueue`.
6. **Plain relative paths that aren't a sibling basename** (`..`, `foo/bar.rs`): resolved against the active folder's host, which keeps the historical `..`-escape rejection for relative inputs. Only _absolute_ paths reach the out-of-workspace branch.

Wiring the same routing into `read_file`, `list_dir`, `write_file`, and `edit_file` means the model can address any bound folder — or any absolute path the bash target can reach — via whichever of those tools the task calls for. `grep` and `bash` aren't path-routed: `grep` always searches the active folder and `bash` runs commands in the active folder's working directory — both stay scoped to `cx.folder`, and a sub-agent against the target folder is the way to do searches or commands there.

#### Out-of-workspace primitives

The `OutOfWorkspace` path is served by helpers on `ToolRegistry` ([`tools.rs`](../crates/moon-coder/src/tools.rs)) that pick container vs. host with the same [`resolve_bash_target`](../crates/moon-coder/src/tools.rs) probe `bash` uses (honouring the per-session force-host override):

- **read**: container → `docker exec <name> cat -- <path>` (direct exec, no shell, so the path can't be word-split or glob-expanded), with mtime from a separate `stat -c %Y` exec; host → [`moon_core::read_host_file`](../crates/moon-core/src/host.rs). Binary detection is the same NUL-in-first-8kB heuristic as the in-workspace path.
- **write**: container → `docker exec -i <name> cp /dev/stdin <path>` with the content piped on stdin; host → [`moon_core::write_host_file`](../crates/moon-core/src/host.rs). Raw bytes either way.
- **list**: container → a single `find -maxdepth 1 -mindepth 1 -printf '%y\t%f\n'` exec (one round-trip, type tag per child so we don't `stat` each entry); host → a `tokio::fs::read_dir` walk. Both emit the same `DirEntry` shape `read_dir` produces, with the child's absolute path in the `path` field so the model can feed it straight back into another tool call.
