...
| ------------ | ------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `read_file`  | `(path) -> { content, encoding, mtime_ms }`                   | Refuses paths outside the active workspace folder                          |
| `write_file` | `(path, content) -> { mtime_ms }`                             | Warns on `dirty` open buffer; the agent picks                              |
| `edit_file`  | `(path, find, replace, occurrence?) -> { mtime_ms, applied }` | `find`/`replace` are exact strings; failure throws so the LLM can retry    |
| `list_dir`   | `(path) -> DirEntry[]`                                        | Honours the same gitignore-aware walk the file tree uses                   |
| `grep`       | `(pattern, path?, glob?) -> Match[]`                          | Backed by the existing `ignore`/ripgrep dep                                |
| `bash`       | `(cmd, cwd?, timeout_ms?) -> { stdout, stderr, exit_code }`   | `WorkspaceHost::spawn`. Inside-container if the workspace is containerised |
