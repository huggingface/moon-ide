...
| `edit_file`  | `(path, find, replace, occurrence?) -> { path, bytes_written, mtime_ms, occurrence, total_matches, match_mode }`    | `find` is matched against the file in four stages: (1) exact byte substring; (2) once more after unescaping literal `\n` / `\t` in `find` (escape-leakage from the model's tool-call JSON); (3) line-aligned indent-tolerant — strip per-line leading whitespace from both sides, find a line-aligned window, splice the file's original byte range and re-indent `replace` to match the file's indent at that point; (4) whitespace-collapsing — normalise every run of ASCII whitespace context-sensitively (kept as one space between word chars, dropped adjacent to punctuation / EOL) on both sides, find the match, splice the file's original byte range and drop `replace` in verbatim. Stage 4 catches the "formatter rewrote a multi-line call into one line" failure mode that stage 3 misses. `match_mode` in the result is `"exact"` / `"fuzzy_unescape"` / `"fuzzy_indent"` / `"fuzzy_whitespace"` so the model can tell which path took. Empty `find` rejected. Non-unique match without `occurrence` throws (the error lists matching line numbers so the model can disambiguate). Same path routing as `read_file`. Fuzzy paths trust format-on-save (editorconfig pre-save + lint-staged) to normalise the spliced bytes — the model still aims for byte-exact, fuzzy is the safety net. |
