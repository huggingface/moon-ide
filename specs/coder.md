...
| ---------------- | --------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `read_file`      | `(path, start_line?, end_line?) -> { content, start_line, end_line, total_lines, truncated, mtime_ms }`                     | `content` is line-numbered: every line is prefixed with `<line_no>\|<line>` (right-aligned, width sized to the largest visible number). The prefix is metadata, not part of the file. `start_line` / `end_line` are 1-based and inclusive; `end_line` is clamped to EOF and the response echoes the _effective_ range so the model can detect short reads. Refuses paths outside the active workspace folder; cross-folder paths (`/workspace/<other>/...` or just `<other>/...`) error with a sub-agent suggestion (see [Synthetic `/workspace/<name>` paths](#synthetic-workspacename-paths-and-cross-folder-rejection)). |
| `write_file`     | `(path, content) -> { path, bytes_written, mtime_ms }`                                                                      | Creates parents only if they exist; agent does `bash mkdir -p` first when it needs to. Same cross-folder gate as `read_file`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `edit_file`      | `(path, find, replace, occurrence?) -> { path, bytes_written, mtime_ms, occurrence, total_matches }`                        | `find` is an exact substring (whitespace significant); empty `find` rejected; non-unique match without `occurrence` throws so the LLM retries with more context. Same cross-folder gate as `read_file`.                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `list_dir`       | `(path) -> DirEntry[]`                                                                                                      | Honours the same gitignore-aware walk the file tree uses. Same cross-folder gate as `read_file`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
The parent's system prompt (rebuilt on every turn in [`runner::refresh_system_prompt`](../crates/moon-coder/src/runner.rs)) gains a "Bound folders" section listing every bound folder with its synthetic `/workspace/<name>` path and a 2–3 sentence description. Descriptions are generated by the `fast` model from each folder's manifest files in canonical order (`AGENTS.md`, `README.md`, `Cargo.toml`, `package.json`, `pyproject.toml`), cached at `<XDG_DATA_HOME>/moon-ide/folder-summaries/<slug>.json` keyed on a 64-bit FNV-1a of the inputs, and invalidated when any of those inputs change. AGENTS.md leads the bundle because it's literally written for agents — when both AGENTS.md and a README exist, the agent guidance anchors the prompt before the user-facing prose. Casing is matched case-insensitively against the folder's top-level entries, so `Readme.md` / `AGENTS.MD` / `Agents.md` all resolve without a hardcoded variant list.
...
### Synthetic `/workspace/<name>` paths and cross-folder rejection

The system prompt advertises every bound folder under a synthetic `/workspace/<name>` path, regardless of where the folder actually lives on disk and regardless of host vs. container mode. The active folder is also exposed under that surface, so `read_file({path: "src/foo.rs"})` and `read_file({path: "/workspace/<active-name>/src/foo.rs"})` resolve identically. Two reasons the synthetic path matters:

1. **Lexical detection.** The model trying to peek at a sibling folder almost always writes `/workspace/<other-name>/...` or just `<other-name>/...`. Both forms are unambiguous: the first starts with `/workspace/` and the second's first segment is a known bound-folder basename. [`tools::ToolRegistry::resolve_workspace_path`](../crates/moon-coder/src/tools.rs) catches both before the path ever reaches the host's resolver and surfaces a `CoderError::ToolFailed` with copy that names the folder and tells the model to call `spawn_subagent` with `folder: "<other-name>"`. Wiring the same nudge into `read_file`, `list_dir`, `write_file`, and `edit_file` means whichever entry point the model picks gets the same coaching.
2. **Disambiguation opt-out.** A directory inside the active folder might legitimately share a sibling's basename. Prefixing with `./` (so `./<other-name>/foo.rs`) skips the cross-folder check entirely; the path passes through as-is and the host's normal resolver runs. This keeps the heuristic strict by default without breaking real layouts.

`grep` and `bash` aren't path-checked: `grep` doesn't take a path (it always searches the active folder), and `bash` runs arbitrary commands where the model could `cd ../<sibling>` no matter what we do — that's a wider problem out of scope for the path gate.

Sub-agents go through the same path gate, scoped against their own assigned folder. They cannot reach the parent's other bound folders (and they can't `spawn_subagent` themselves either — depth=1 is enforced by tool-list filtering at the parent's `run_turn`).
